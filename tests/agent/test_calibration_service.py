...
def _write_robot(root: Path) -> None:
    ...
    for side, lateral, roll_sign in (("left", 0.18, 1.0), ("right", -0.18, -1.0)):
        ...
        shoulder_lower, shoulder_upper = (
            (-0.1, 2.2) if roll_sign > 0 else (-2.2, 0.1)
        )
        xml.extend(
            [
                f'<link name="{upper}"/>',
...
def _service(tmp_path: Path) -> tuple[CalibrationService, str, Path]:
    ...
    assets = AgentAssetService(
        AssetRegistry(tmp_path / "agent-state", {"robots": robots})
    )
...
def test_status_marks_an_existing_but_arm_down_t_pose_calibration_invalid(
    ...
    assert (
        ...
        / "agent-state"
        / "calibration-history"
        / f"{status.calibration_digest}.yaml"
    ).is_file()
