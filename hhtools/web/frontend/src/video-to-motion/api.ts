...
export function summarizeMotionResult(
  ...
  const positionsLength = Array.isArray(result.positions)
...
export interface VideoToMotionJob {
  ...
  readonly status: VideoToMotionJobState;
...
export interface StartVideoToMotionInput {
  readonly video: File;
  readonly staticCamera: boolean;
  readonly focalLength?: number;
}

export interface WaitForVideoToMotionOptions {
  readonly signal: AbortSignal;
  readonly onUpdate: (job: VideoToMotionJob) => void;
...
export async function waitForVideoToMotion(
  jobId: string,
  options: WaitForVideoToMotionOptions,
  fetcher: Fetcher = fetch,
): Promise<MotionResult> {
  const interval = Math.max(0, options.pollIntervalMs ?? 500);

  for (;;) {
    const job = await requestJson<VideoToMotionJob>(
      `/api/job/${encodeURIComponent(jobId)}`,
      { signal: options.signal },
      fetcher,
    );
    const normalized = { ...job, progress: boundedProgress(job.progress) };

    if (normalized.kind !== "video_to_motion") {
      throw new Error(
        `Job ${normalized.id} has kind ${normalized.kind}; expected video_to_motion.`,
      );
    }
    options.onUpdate(normalized);

    if (normalized.status === "done") {
      if (normalized.scope === "persistent") {
        throw new Error("This completed job no longer has a live motion result.");
      }
      if (!normalized.result) {
        throw new Error("The job completed without a motion result.");
      }
      ...
      throw new Error(
        normalized.error ||
          (normalized.status === "cancelled"
            ...
            : "Video-to-motion failed."),
      );
    }
    if (normalized.status !== "pending" && normalized.status !== "running") {
      throw new Error(`Job ${normalized.id} returned an unknown status.`);
    }
