...
describe('WorkflowPipeline', () => {
  ...
  it('localizes default and runtime node labels when the locale changes', async () => {
    const wrapper = mount(WorkflowPipeline, {
      props: { workflow: 'h2r', locale: 'en' },
    })
    mountedWrappers.push(wrapper)

    expect(wrapper.attributes('aria-label')).toBe('Human to Robot pipeline')
    ...
    expect(wrapper.attributes('aria-label')).toBe('人体到机器人流程')
    expect(wrapper.findAll('.workflow-node-label').map((node) => node.text())).toEqual([
      '动作',
      '机器人',
      '标定',
