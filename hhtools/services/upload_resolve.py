...
def _pick_primary_clip(
    ...
    rels = [str(r).replace("\\", "/").lstrip("/") for r in prefer_paths if str(r or "").strip()]
    for rel in rels:
...
def _load_via_dataset_adapter(path: Path, dataset: str, *, progress=None):
    ...
    adapter_factories: dict[str, tuple] = {
        ...
        "gvhmr": (
            lambda: __import__("hhtools.io.datasets.hmr4d", fromlist=["GvhmrAdapter"]).GvhmrAdapter,
            False,
        ),
        "kungfu_athlete": (
            lambda: (
...
def _load_mimic(  # noqa: PLR0911 - dataset loaders intentionally return at detection points
    path: Path,
    load_motion_file,
    load_via_adapter,
    *,
...
def _infer_dataset_from_path(  # noqa: PLR0911 - explicit dataset decision table
    path: Path,
    profile: str,
    *,
    clip_kind: str = "",
    ...
    from hhtools.services.motion_library import _DIR_TO_ADAPTER, _normalise_dirname
