...
if TYPE_CHECKING:
    from hhtools.services.motion_library import LibraryEntry
...
def _common_path_parts(rels: list[str]) -> tuple[str, ...]:
    ...
    for rel in posix_rels[1:]:
        common = tuple(a for a, b in zip(common, rel.parts, strict=False) if a == b)
        if not common:
            break
    return common
...
def _pick_best_clip_candidate(
    ...
    if not candidates or not hints:
        return None
    scored = [(path, _score_clip_candidate(path, hints, folder_label)) for path in candidates]
...
def resolve_clip_on_disk(  # noqa: PLR0911 - each source boundary has a distinct fallback
    ...
    *,
    ...
    if recorded_resolved is not None and _recorded_path_matches_hints(
        ...
        scores = {path: _score_clip_candidate(path, hints, folder_label) for path in candidates}
...
def _upload_tree_root(drop_dir: Path) -> Path:
    ...
    children = sorted(p for p in drop_dir.iterdir() if p.is_dir() and not p.name.startswith("."))
...
def scan_motions_library(
    ...
    from hhtools.services.dataset_catalog import build_analysis_entries
    ...
    for child in sorted(root.iterdir()):
        ...
        try:
            raw_entries = build_analysis_entries(child)
        except Exception:
            continue
