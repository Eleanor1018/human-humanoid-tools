...
| `ambiguous start` | Replay the exact same `start_retarget` request with the same `plan_id` and idempotency key, then query the returned job. | Do not substitute `retry_job`, generate a new key, or assume the first start failed. |
