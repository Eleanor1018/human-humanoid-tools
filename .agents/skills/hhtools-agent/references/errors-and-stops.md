...
| `CALIBRATION_REQUIRED` / `CALIBRATION_MISMATCH` | Treat the structured `next_action` as a human handoff, then create a new preflight after confirmation. | Do not patch a calibration or silently choose another reference. |
...
| `transport ambiguity` | Replay the exact same `start_retarget` request with the same `plan_id` and idempotency key, then query the returned job. | Do not call `retry_job`, generate a new key, or assume the first start failed. |
...
- Transport replay repeats the same logical submission with the same `plan_id` and idempotency
  key because it is unknown whether the original response arrived.
