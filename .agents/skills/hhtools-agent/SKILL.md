...
5. Generate one caller-owned idempotency key for this logical submission. Call
   `start_retarget` with only the ready `plan_id` and that key. Record the exact pair for safe
   replay if the transport result is ambiguous.
...
8. Inspect both `state` and `outcome`. `completed` alone is not quality approval. Present the
   evaluation and manifest; pause on `review_required`, `partial`, or `rejected`.
